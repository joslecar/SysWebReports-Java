...
public class PantallaVerReportes {
    ...
    private Button regresar,buscarFecha,buscarNombre;
    ...
    private void cargarPanes(Stage stage,Vendedor user,List<Reporte> reportes){
        ...
        ObservableList<Reporte> listareportes1 = FXCollections.observableArrayList();
        ...
        buscarFecha.setOnAction(e->{
            ...
            if(fecha1.getValue() == null || fecha2.getValue() == null){
                ...
                try {
                    ...
                    ResultSet r = s.executeQuery(query + " where w.fecha_rs between '"+fecha1.getValue().toString()+"' and '"+fecha2.getValue().toString()+"'");
                    while(r.next()){
       ...
       buscarNombre.setOnAction(e->{
            ...
            if(busquedaNombre.getText().isEmpty()){
                ...
                try {
                    ConexionSQL canal = new ConexionSQL();
                    Connection cn = canal.conectarSQL("SYSWEBSERVICE\\SQLEXPRESS", "data01", "sa", "Rootpass1"); 
                    try (Statement s = cn.createStatement(); ResultSet r = s.executeQuery(query + " where empcli COLLATE SQL_Latin1_General_Cp1_CI_AI like '%"+busquedaNombre.getText()+"%'")) {
                        while(r.next()){
                            List<ReporteDetalle> listadetalles = new ArrayList<>();  
                            Statement s2 = cn.createStatement();
                            ResultSet r2 = s2.executeQuery("select master,a.codigo,nombre,valor,pideval,w.numero_rs from (webservices w join webservices_detail wd on w.numero_rs = wd.numero_rs and w.tipo = wd.tipo) join alptabla a on wd.cod_rubro = a.codigo join "+
                                    "dp05a110 d on w.empresa_rs = d.codigo where w.numero_rs = '"+r.getString("numero_rs")+"'"+" and w.tipo = '"+r.getString("tipo")+"'");
